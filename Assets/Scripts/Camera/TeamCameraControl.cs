...
public class TeamCameraControl : MonoBehaviour
{
    static public TeamCameraControl sInstance;
    ...
    private Vector3 FindAveragePosition()
    {
        ...
        for (int i = 0; i < TeamGameManager.m_Tanks.Count; i++)
        {
            // If the target isn't active, go on to the next one.
            if (!TeamGameManager.m_Tanks[i].m_Instance.activeSelf)
                continue;

            // Add to the average and increment the number of targets in the average.
            average += TeamGameManager.m_Tanks[i].m_Instance.transform.position;
            numTargets++;
        }
    ...
    private float MaxTargetDistance(Vector3 desiredPosition)
    {
        ...
        for (int i = 0; i < TeamGameManager.m_Tanks.Count; i++)
        {
            ...
            float targetDistance = (desiredPosition - TeamGameManager.m_Tanks[i].m_Instance.transform.position).magnitude;
