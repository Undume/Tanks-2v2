...
public class TeamTankSetup : NetworkBehaviour
{
    [Header("UI")]
    ...
    public override void OnStartClient()
    {
        ...
        if (!isServer) //if not hosting, we had the tank to the gamemanger for easy access!
            TeamGameManager.AddTank(gameObject, team, m_PlayerNumber, m_Color, m_PlayerName, m_LocalID);
    ...
    public void Update()
    {
        ...
        if (TeamGameManager.s_Instance.m_GameIsFinished && !m_IsReady)
        {
    ...
    {
        TeamGameManager.s_Instance.RemoveTank(gameObject);
    }
}
