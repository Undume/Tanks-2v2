...
public class TeamGameManager : NetworkBehaviour
{
    static public TeamGameManager s_Instance;

    //this is static so tank can be added even withotu the scene loaded (i.e. from lobby)
    static public List<TeamTankManager> m_Tanks = new List<TeamTankManager>();             // A collection of managers for enabling and disabling different aspects of the tanks.

    public int m_NumRoundsToWin = 5;          // The number of rounds a single player has to win to win the game.
    public float m_StartDelay = 3f;           // The delay between the start of RoundStarting and RoundPlaying phases.
    public float m_EndDelay = 3f;             // The delay between the end of RoundPlaying and RoundEnding phases.
    public TeamCameraControl m_CameraControl;     // Reference to the CameraControl script for control during different phases.
    ...
    private Teams m_RoundWinner;
    private Teams m_GameWinner;           // Reference to the winner of the game.  Used to make an announcement of who won.
    private float m_blueRoundsWon;
    private float m_redRoundsWon;
    ...
    static public void AddTank(GameObject tank, Teams team, int playerNum, Color c, string name, int localID)
    {
        TeamTankManager tmp = new TeamTankManager();
        tmp.m_team = team;
        tmp.m_Instance = tank;
    ...
    private IEnumerator RoundEnding()
    {
        ...
        m_RoundWinner = Teams.None;

        // See if there is a winner now the round is over.
        m_RoundWinner = GetRoundWinner();

        // If there is a winner, increment their score.
        switch (m_RoundWinner)
        {
            case Teams.Blue:
                m_blueRoundsWon++;
                break;
    ...
    private bool OneTankLeft()
    {
        ...
        int numBlueTanksLeft = 0;
        int numRedTanksLeft = 0;

        // Go through all the tanks...
        for (int i = 0; i < m_Tanks.Count; i++)
        {
            // ... and if they are active, increment the counter.
            if (m_Tanks[i].m_TankRenderers.activeSelf && m_Tanks[i].m_team == Teams.Blue)
                numBlueTanksLeft++;
            if (m_Tanks[i].m_TankRenderers.activeSelf && m_Tanks[i].m_team == Teams.Red)
                numRedTanksLeft++;
        }

        // If there are one or fewer tanks remaining return true, otherwise return false.
        return numRedTanksLeft < 1 || numBlueTanksLeft < 1;
    }
    ...
    private Teams GetGameWinner()
    {
        ...
        if (m_blueRoundsWon == m_NumRoundsToWin)
            return Teams.Blue;
        if (m_redRoundsWon == m_NumRoundsToWin)
            return Teams.Red;
        for (int i = 0; i < m_Tanks.Count; i++)
        {
            m_Tanks[i].SetLeader((m_blueRoundsWon > m_redRoundsWon && m_Tanks[i].m_team == Teams.Blue) ||
                (m_redRoundsWon > m_blueRoundsWon && m_Tanks[i].m_team == Teams.Red));
        }
        return Teams.None;
    }
    ...
    private string EndMessage(int waitTime)
    {
        ...
        {
            case Teams.Red:
                message = "<color=#FF0000FF>RED TEAM</color> WINS THE GAME!";
                break;
            case Teams.Blue:
                message = "<color=#0079FFFF>BLUE TEAM</color> WINS THE GAME!";
                break;
            case Teams.None:
                {
                    if (m_RoundWinner == Teams.Blue)
                        message = "<color=#0079FFFF>BLUE TEAM</color> WINS THE ROUND!";
                    else if (m_RoundWinner == Teams.Red)
                        message = "<color=#FF0000FF>RED TEAM</color> WINS THE ROUND!";
                }
                break;
        }
        ...
        message += "<color=#0079FFFF>BLUE TEAM</color> HAS " + m_blueRoundsWon + " ROUNDS!\n";
        message += "<color=#FF0000FF>RED TEAM</color> HAS " + m_redRoundsWon + " ROUNDS!";
